...
const NODE_DOCS_RU: Record<string, any> = {
    ...
    trade_action_sltp: { title: '🛡️ Stop Loss / Take Profit', desc: 'Настройка защиты позиции — стоп-лосс и тейк-профит, трейлинг-стоп, частичные тейки, DCA/усреднение.', logic: 'Подключите ноду после Market/Limit Order. Задайте SL/TP в процентах или ATR-множителях; включите Trailing Stop, Partial TP и/или DCA-уровни при необходимости.' },
    trade_action_risk: { title: '🛡️ Risk Guard', desc: 'Ограничитель риска — блокирует новые входы при превышении лимитов просадки или экспозиции.', logic: 'Задайте maxDrawdown и maxExposure. Нода не открывает сделки сама — она встраивается в цепочку условий и прерывает вход, если лимит превышен.' },
    trade_action_sizing: { title: '⚖️ Risk Sizing', desc: 'Расчёт размера позиции по выбранному методу (fixed / ATR / % от equity / Kelly).', logic: 'Выберите метод расчёта и параметры (риск %, ATR-множитель и т.д.) — размер позиции будет вычисляться динамически при каждом входе вместо фиксированного объёма.' },
    trade_action_telegram: { title: '📨 Telegram Alert (легаси)', desc: 'Отправляет текстовое уведомление о сигнале через ОБЩИЙ Telegram-бот, настроенный в Settings.', logic: 'Шлёт всем сигналам одно и то же сообщение через общий бот из настроек — без маршрутизации по стратегиям. Для отдельного чата/бота на стратегию используйте новую ноду ✈️ Telegram (категория «Выходные») с настройкой через Подключения.' },
    trade_action_grid: { title: '🔲 Grid Bot', desc: 'Сеточный бот — расставляет серию лимитных ордеров в ценовом диапазоне.', logic: 'Задайте нижнюю/верхнюю границу диапазона, количество сеток и тип (арифметическая/геометрическая).' },
    telegram_output: { title: '✈️ Telegram', desc: 'Доставка сигнала в конкретный Telegram-чат через сохранённое Подключение — маршрут для ЭТОЙ стратегии, не общий бот.', logic: 'Выберите Подключение (Настройки → Подключения), укажите chat id и, при желании, свой текст сообщения с плейсхолдерами {{pair}} {{signal}} {{price}}.' },
    discord_output: { title: '🎮 Discord', desc: 'Доставка сигнала в Discord-канал через сохранённый webhook.', logic: 'Выберите Подключение типа Discord Webhook. По умолчанию отправляется оформленный embed; можно переключить на простой текст.' },
    webhook_output: { title: '🔗 Webhook', desc: 'Доставка сигнала на произвольный HTTP-эндпоинт (свой сервер, Zapier, n8n и т.д.).', logic: 'Выберите Подключение типа Webhook. Можно задать свой JSON-шаблон payload и включить HMAC-подпись для верификации на своей стороне.' },
    testnet_trading_output: { title: '🔑 Testnet Trading', desc: 'Реальное исполнение ордеров на бирже, но в sandbox/testnet-режиме — без риска для настоящих средств.', logic: 'Укажите биржу и API-ключи ТЕСТНЕТА (не боевые!). Ордера идут через ccxt-queue с тем же рейт-лимитом, что и боевая торговля — удобно проверить исполнение перед реальным запуском.' },
    paper_trading_output: { title: '🧪 Paper Trading', desc: 'Виртуальный торговый счёт — симулирует сделки на реальных ценах без реальных денег.', logic: 'Задайте стартовый капитал, плечо и риск на сделку — счёт ведёт отдельную историю сделок и P&L для этой конкретной ноды на канвасе.' },
    signal_publisher_output: { title: '📢 Signal Publisher', desc: 'Публикует торговый сигнал в Ленту — публичный профиль, доступный другим пользователям для копитрейдинга.', logic: 'Подключите после сигнала — при срабатывании стратегии пост с сигналом появится в вашей Ленте, если включена публичность.' },
...
const NODE_DOCS_EN: Record<string, any> = {
    ...
    trade_action_sltp: { title: '🛡️ Stop Loss / Take Profit', desc: 'Position protection settings — stop-loss and take-profit, trailing stop, partial TPs, DCA/rebuy averaging.', logic: 'Connect this node after Market/Limit Order. Set SL/TP as a percent or ATR multiplier; enable Trailing Stop, Partial TP and/or DCA levels as needed.' },
    trade_action_risk: { title: '🛡️ Risk Guard', desc: 'A risk limiter — blocks new entries once drawdown or exposure limits are exceeded.', logic: 'Set maxDrawdown and maxExposure. This node does not open trades itself — it sits in the condition chain and interrupts entry once the limit is breached.' },
    trade_action_sizing: { title: '⚖️ Risk Sizing', desc: 'Computes position size using the selected method (fixed / ATR / % of equity / Kelly).', logic: 'Choose the sizing method and its parameters (risk %, ATR multiplier, etc.) — position size is computed dynamically on each entry instead of a fixed volume.' },
    trade_action_telegram: { title: '📨 Telegram Alert (legacy)', desc: 'Sends a text notification through the SINGLE global Telegram bot configured in Settings.', logic: 'Every signal goes through the one shared bot from settings — no per-strategy routing. For a dedicated chat/bot per strategy, use the new ✈️ Telegram node (Outputs category) configured via Connections.' },
    trade_action_grid: { title: '🔲 Grid Bot', desc: 'A grid bot — places a series of limit orders across a price range.', logic: 'Set the lower/upper range bounds, number of grids, and type (arithmetic/geometric).' },
    telegram_output: { title: '✈️ Telegram', desc: 'Delivers the signal to a specific Telegram chat via a saved Connection — a per-strategy route, not the shared bot.', logic: 'Pick a Connection (Settings → Connections), set the chat id, and optionally a custom message template with {{pair}} {{signal}} {{price}} placeholders.' },
    discord_output: { title: '🎮 Discord', desc: 'Delivers the signal to a Discord channel via a saved webhook.', logic: 'Pick a Discord Webhook Connection. Sends a formatted embed by default; can be switched to plain text.' },
    webhook_output: { title: '🔗 Webhook', desc: 'Delivers the signal to any HTTP endpoint (your own server, Zapier, n8n, etc).', logic: 'Pick a Webhook Connection. You can set a custom JSON payload template and enable HMAC signing to verify it on your side.' },
    testnet_trading_output: { title: '🔑 Testnet Trading', desc: 'Real order execution on the exchange, but in sandbox/testnet mode — no risk to real funds.', logic: 'Provide the exchange and TESTNET API keys (not live!). Orders go through the same rate-limited ccxt-queue as live trading — useful for verifying execution before going live.' },
    paper_trading_output: { title: '🧪 Paper Trading', desc: 'A virtual trading account — simulates trades at real prices without real money.', logic: 'Set starting capital, leverage and risk per trade — the account keeps its own trade history and P&L for this specific node on the canvas.' },
    signal_publisher_output: { title: '📢 Signal Publisher', desc: 'Publishes the trading signal to the Feed — a public profile other users can copy-trade from.', logic: 'Connect after the signal — when the strategy triggers, a signal post appears in your Feed if publicity is enabled.' },
...
const PropertiesPanel = ({ selectedNode, onUpdate, onDelete, nodeCount = 0, pair, timeframe }: Props) => {
  ...
  const getDocs = () => {
    ...
    if (originalType === 'trade_action') {
        ...
        if (data.action === 'sltp') return docsSource['trade_action_sltp'];
        if (data.action === 'risk') return docsSource['trade_action_risk'];
        if (data.action === 'sizing') return docsSource['trade_action_sizing'];
        if (data.action === 'telegram') return docsSource['trade_action_telegram'];
        if (data.action === 'grid') return docsSource['trade_action_grid'];
        return docsSource['trade_action']; // market_order / limit_order
    }
